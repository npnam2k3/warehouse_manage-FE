...
import { getAllExportOrder } from "../../apis/exportOrderService";

import TableExportOrder from "./components/TableExportOrder";
import ModalCreateExportOrder from "./components/ModalCreateExportOrder";
...
const ExportOrders = () => {
  ...
  const [openModalCreateExportOrder, setOpenModalCreateExportOrder] =
    useState(false);
  ...
  const handleClickCreateButton = () => {
    setOpenModalCreateExportOrder(true);
  };

  const fetchData = async () => {
    setIsLoading(true);
    try {
      const res = await getAllExportOrder({
        limit,
      ...
      setExportOrders(res.data?.data?.orders);
  ...
  return (
    <Box sx={{ bgcolor: "#F0F1FA", minHeight: "100vh", p: 3 }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between" }}>
        ...
        >
          <AddIcon />
          <Typography variant="span">Tạo mới hóa đơn bán hàng</Typography>
        </Button>
      </Box>
      ...
      {openModalCreateExportOrder && (
        <ModalCreateExportOrder
          open={openModalCreateExportOrder}
          setOpen={setOpenModalCreateExportOrder}
          fetchData={fetchData}
          setPage={setPage}
        />
      )}
