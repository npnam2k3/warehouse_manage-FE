...
const ModalPaymentImportOrder = ({
  ...
  const handleOnSubmit = async (dataSubmit) => {
    if (
      ...
      dataSubmit.amount < import_order?.amount_due
    ) {
      setError("amount_invalid", {
        type: "manual",
        message: `Số tiền phải thanh toán toàn bộ là ${import_order?.amount_due}`,
      });
      return;
    }
    const payload = {
      type: TypePayment.IMPORT,
      list_orders: [
        {
  ...
  useEffect(() => {
    if (
      watch("amount") >= import_order?.amount_due &&
  ...
  return (
    ...
    >
      <DialogTitle>
        ...
        <strong>{import_order?.import_order_code}</strong>
      </DialogTitle>
      <DialogContent>
        <Typography>
          Đã trả: <strong>{import_order?.amount_paid}</strong>
        </Typography>
        <Typography>
          Còn lại: <strong>{import_order?.amount_due}</strong>
        </Typography>
          ...
          <Box sx={{ mb: "24px" }}>
            <TextField
              ...
              {...register("amount", {
                ...
                max: {
                  value: import_order?.amount_due || 0,
                  message: `Số tiền thanh toán không được vượt quá số tiền đang nợ: ${import_order?.amount_due}`,
                },
              })}
            />
